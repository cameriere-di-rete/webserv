...
void Config::translateLocationBlock_(const BlockNode& location_block,
  ...
  for (size_t i = 0; i < location_block.directives.size(); ++i) {
    ...
    if (directive.name == "root") {
      ...
      requireArgsEqual_(directive, 1);
      loc.autoindex = parseBooleanValue_(directive.args[0]) ? ON : OFF;
    ...
    } else if (directive.name == "cgi_root") {
      requireArgsEqual_(directive, 1);
      loc.cgi_root = directive.args[0];
      LOG(DEBUG) << "  Location CGI root: " << loc.cgi_root;
    } else if (directive.name == "max_request_body") {
      requireArgsEqual_(directive, 1);
      loc.max_request_body = parsePositiveNumber_(directive.args[0]);
