...
Location Server::matchLocation(const std::string& path) const {
  ...
  // max_request_body: inherit from server if location still has default
  // and server has a different (explicitly set) value
  if (result.max_request_body == DEFAULT_MAX_REQUEST_BODY &&
      max_request_body != DEFAULT_MAX_REQUEST_BODY) {
